...
- `decision_factors` must only use values from this fixed list: {vocab_list}. Omit any that don't apply; never invent new ones. Every decision factor you list must be backed by at least one corresponding phrase in `reasons_for_saving`, `blockers_to_purchase`, or `info_sought_outside_app` explaining why it applies to this item — never list a decision factor with no explanatory phrase anywhere in the output (found live 2026-08-30: items were getting `decision_factors: ["fit"]` with all three phrase fields left empty, which is unusable downstream since nothing describes the actual fit concern).
